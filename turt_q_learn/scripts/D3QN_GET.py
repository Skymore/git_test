...
import rospy, datetime, random, math, os, tensorflow as tf, numpy as np, matplotlib.pyplot as plt, keras
...
class envWrapper():
    ...
    def getState(self, ranges):  # read laser ranges, get goal info
        ...
        ranges = [(self.maxScanRange if str(r) == 'inf' else min(r, self.maxScanRange)) for i, r in enumerate(ranges) if
                  not i % self.scanRatio]
    ...
    def resetState(self,doneWithGoal):  # respawn turtle and goal models, get initial state
        ...
        return self.getState(self.getScan())
    ...
    def getReward(self,
        ...
        if state[len(state) - 2] < GOAL_MIN_DISTANCE:
            return self.goalReward, False, True
...
def genModel(optimizer, loss, lr, first, hidden, last, stateSpace, initializer):
    # 第一部分
    input = tf.keras.layers.Input(shape=(stateSpace,))
    h1 = tf.keras.layers.Dense(32, input_shape=(stateSpace,), activation=first, kernel_initializer=initializer)(input)
    h2 = tf.keras.layers.Dense(32, input_shape=(stateSpace,), activation=hidden, kernel_initializer=initializer)(h1)
    h3 = tf.keras.layers.Dense(16, input_shape=(stateSpace,), activation=hidden, kernel_initializer=initializer)(h2)
    # 第二部分
    svalue = tf.keras.layers.Dense(1, activation=last)(h3)
    # 第三部分
    avalue = tf.keras.layers.Dense(len(ACTION_SPACE), activation=last)(h3)  # 计算avalue
    mean = tf.keras.layers.Lambda(lambda x: tf.reduce_mean(x, axis=1, keepdims=True))(avalue)  # 用Lambda层，计算avg(a)
    advantage = tf.keras.layers.ElementwiseLambda(lambda x, y: x - y)([avalue, mean])  # a - avg(a)
    output = tf.keras.layers.ElementwiseLambda(lambda x, y: x + y)([advantage, svalue])
...
if __name__ == '__main__':
    ...
    try:
        ...
        hyperParameterList = {  # set hyperparameter combinations
            ...
            "Gamma": [.99],
            "Scan Ratio": [12, 18],  # how wany of the 360 scans are read, larger number = less scan count
