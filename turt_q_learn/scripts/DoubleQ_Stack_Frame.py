...
class envWrapper():
    ...
    def __init__(self, params):  # Set required parameters
        ...
        self.numStackScan = params["Num Stack Scan"]
    ...
    def stackScan(self):
        dataStack = []
        for i in range(self.numStackScan):
            data = None
            while data is None:
                try:
    ...
    def step(self, action):  # publish action and read new state, reward, and done variables
        ...
        dataStack = self.stackScan()
        newState = self.getState(dataStack)
    ...
    def resetState(self):  # respawn turtle and goal models, get initial state
        ...
        dataStack = self.stackScan()
        return self.getState(dataStack)
...
def genModel(optimizer, loss, lr, first, hidden, last, stateSpace, initializer):  # build neural net to be used for DDQN
    ...
    model.add(tf.compat.v1.keras.layers.Dense(32, input_shape=(stateSpace,), activation=first,
...
if __name__ == '__main__':
    ...
    try:
        ...
        if hyperParameterList["Num Stack Scan"] > 1
            hyperParameterList["State Space"] = [(360 // r) * f + 3 for r in hyperParameterList["Scan Ratio"]
                                                 for f in hyperParameterList["Num Stack Scan"]]
        else:
