...
ROLLING_AVERAGE_SAMPLES = 20  # number of samples used for rolling average graph
...
class modelClass():
    ...
    def __init__(
        ...
        self.modelType = hyperParams["Model Type"]
        if hyperParams["Load Model"]:
            ...
            self.model = genModel(hyperParams["Optimizer"], hyperParams["Loss"], hyperParams["Learning Rate"],
                                  ...
                                  hyperParams["Initializer"], self.modelType)
        ...
        self.actionRate = hyperParams["Action Rate"]
        self.turt_q_learn_path = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    ...
    def playEpisodes(self):  # complete set amount of episodes, return numpy array of scores from each
        self.tensorboard = TensorBoard(log_dir='~/tensorlog/{}/'.format(self.saveName))
        scores = []
        success = []
        totalStep = 0
        maxStep = 30000
        for epNum in range(self.episodes):
            ...
            actionRate = rospy.Rate(self.actionRate)
            ...
            for stepNum in range(
                ...
                if (stepNum > 0): # 第一次执行动作时不存储经验
                    transition = np.append(lastState, lastAction)
                    transition = np.append(transition, state)
                ...
                if done:
                    break
                if stepNum == self.episodeLength-1:
                    print("Time out!")
        ...
        plt.figure(20, figsize=(10,2), dpi=300)
        averages = movingAverage(scores)
        success_rate = movingAverage(success) * 100
        print("success_rate of last 50 eps", success_rate)

        plt.subplot(211)
        plt.plot(scores, label="Scores")
        plt.plot(np.arange(ROLLING_AVERAGE_SAMPLES - 1, epNum+1), averages, label="Average")
        plt.ylabel("Scores")

        plt.subplot(212)
        plt.plot(np.arange(ROLLING_AVERAGE_SAMPLES - 1, epNum+1), success_rate)#, label="Success Rate")
    ...
    def train(self):
        ...
        X_batch, Y_batch = self.get_batch_fast()
        #print(X_batch.shape, Y_batch.shape)
        #t2 = time.time()
        #print("Bat time: {0}".format(t2-t1))
        self.model.fit(X_batch, Y_batch, verbose=0)#, callbacks=self.tensorboard)
        #t3 = time.time()
...
class envWrapper():
    ...
    def __init__(self, params):  # Set required parameters
        ...
        self.scanRewardScalar = params["Scan Reward Scalar"]
...
def genModel(optimizer, loss, lr, first, hidden, last, stateDim, actionDim, initializer, modelType):  # build neural net to be used for DDQN
    # 三层
    print(modelType)
    if modelType == 0:
        model = tf.keras.models.Sequential()
        model.add(Dense(32, input_shape=(stateDim,), activation=first,
                                                  kernel_initializer=initializer))
        model.add(Dense(32, activation=hidden, kernel_initializer=initializer))
        model.add(Dense(16, activation=hidden, kernel_initializer=initializer))
        model.add(Dense(actionDim, activation=last))
    ...
    # 三层dueling.分支在第二层之后
        ...
        inputs = Input(shape=(stateDim,))
        x = Dense(32, input_shape=(stateDim,), activation=first, kernel_initializer=initializer)(inputs)
        x = Dense(32, activation=hidden, kernel_initializer=initializer)(x)
        x = Dense(16, activation=hidden, kernel_initializer=initializer)(x)
        value = Dense(actionDim, activation=last, kernel_initializer=initializer)(x)
        advantage = Dense(actionDim, activation=last, kernel_initializer=initializer)(x)
        aMean = Lambda(lambda x: K.mean(x, axis=1, keepdims=True))(advantage)
        advantage = Subtract()([advantage, aMean])
...
if __name__ == '__main__':
    ...
    try:
        ...
        plt.xlabel("Episodes")
        plt.legend()
        plt.savefig(folderPath + '/plot2.png', bbox_inches='tight')
        plt.close()
        ...
        plt.xlabel("Episodes")
        plt.legend()
        plt.savefig(folderPath + '/plot3.png', bbox_inches='tight')
        plt.close()
